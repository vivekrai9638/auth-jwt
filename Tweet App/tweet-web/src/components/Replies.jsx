...
import { likeTweetAsync } from "../slices/tweetSlice";
...
const Replies = ({ replies }) => {
  ...
  const handleLike = (e, reply) => {
    e.preventDefault();
    dispatch(
      likeTweetAsync({ username: user.userName, tweetId: reply.id, token })
    );
  };
    ...
    <div>
      {replies?.map((reply) => {
        ...
        return (
          <Link
            key={reply.id}
            to={`/${reply.user.userName}/status/${reply.id}`}
            style={{ textDecoration: "none" }}
          >
            <PeopleFlex hover key={reply.id} border={theme.border}>
              <div>
                <UserImage src={reply.user.avatar} />
              </div>
              <div style={{ width: "100%" }}>
                ...
                <div style={{ color: theme.color }}>{reply.message}</div>
              </div>
              {/* <TweetDetails style={{ justifyContent: "end" }}>
                ...
                <ActivityBox
                  onClick={(event) => handleLike(event, reply)}
